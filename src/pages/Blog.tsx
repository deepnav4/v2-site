      ...
      <div className="container py-16">
        ...
        <div className="space-y-0">
          {filteredPosts.map(post => (
            <Link 
              key={post.id}
              to={`/blog/${post.slug}`}
              className={`block py-8 border-b transition-colors duration-200 group ${
                theme === 'dark' 
            ...
            >
              <article>
                <div className="flex items-baseline gap-3 mb-3">
                  <time className={`text-sm font-sans ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'}`}>
                    {post.date}
                  </time>
                  <span className={`text-sm font-sans ${theme === 'dark' ? 'text-gray-600' : 'text-gray-400'}`}>
                    •
                  </span>
                ...
                <h2 className={`text-2xl font-normal mb-3 transition-colors duration-200 font-serif group-hover:text-emerald-500 ${
                  theme === 'dark' ? 'text-white' : 'text-black'
                }`}>
                  {post.title}
                </h2>
                
                <p className={`text-base leading-relaxed font-sans ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
                  {post.excerpt}
                </p>
              </article>
            </Link>
        ...
        {filteredPosts.length > 10 && (
          <div className="mt-16 text-center">
            <button className={`px-6 py-3 rounded-lg font-medium transition-all duration-200 font-sans ${
              theme === 'dark'
