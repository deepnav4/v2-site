...
export default function Tags() {
  ...
  return (
    <div className="container py-16">
      ...
      <div
        ref={headerRef.ref}
        className={`max-w-3xl mb-16 reveal ${headerRef.isVisible ? 'visible' : ''}`}
      >
        <h1 className={`text-4xl md:text-5xl font-bold tracking-tight mb-4 reveal stagger-1 ${headerRef.isVisible ? 'visible' : ''}`}>
          Explore by Topic
        </h1>
        <p className={`text-lg reveal stagger-2 ${headerRef.isVisible ? 'visible' : ''} ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
      ...
      <div
        ref={tagsRef.ref}
        className={`max-w-5xl reveal ${tagsRef.isVisible ? 'visible' : ''}`}
      >
          ...
          {allTags.map((tag, index) => (
            <Link
              ...
              className={`group inline-flex items-center gap-2 px-5 py-3 rounded-full
                transition-all duration-300 border reveal ${tagsRef.isVisible ? 'visible' : ''}
                ${theme === 'dark'
